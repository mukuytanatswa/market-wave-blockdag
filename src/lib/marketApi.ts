...
// API Configuration
const ALPHAWAVE_API_KEY = import.meta.env.VITE_ALPHAWAVE_API_KEY;
...
export const fetchCryptoData = async (limit = 10): Promise<CoinData[]> => {
  try {
    const apiKey = COINGECKO_API_KEY ? `&x_cg_demo_api_key=${COINGECKO_API_KEY}` : '';
    const response = await fetch(
      `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=${limit}&page=1&sparkline=true&price_change_percentage=24h${apiKey}`
    );
